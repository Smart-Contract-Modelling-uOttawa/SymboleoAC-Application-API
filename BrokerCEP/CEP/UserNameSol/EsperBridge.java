...
public class EsperBridge {
    ...
    // Build a TLS-encrypted connection to RabbitMQ using username/password auth
private static com.rabbitmq.client.Connection getRabbitConnection() throws Exception {
    ...
    // Load CA certificate (optional with verify_none, but good for consistency)
    ...
    ts.load(null, null);
    ts.setCertificateEntry("fabric-ca", caCert);
    ...
    factory.enableHostnameVerification(); // optional with verify_none

    // Use PLAIN authentication
    factory.setUsername("cep_bridge");
    factory.setPassword("strongpassword123");

    // 🚫 Do NOT use EXTERNAL now
    // factory.setSaslConfig(DefaultSaslConfig.EXTERNAL);

    System.out.println("🔐 TLS + PLAIN auth configured.");
